...
BOT_TOKEN = '8441255242:AAERASmFy6D8Ru2SV1D4ndssyyBpcwcNJDI'
ADMIN_IDS = [7887884775, 7564889663]  # 📌 NHIỀU ADMIN
...
def init_user_state(chat_id):
    if chat_id not in user_states:
        user_states[chat_id] = {
            ...
            "loss_streak": 0, "base_bet_amount": 10000, "current_bet": 10000,
...
def start_websocket(chat_id, token, is_background=False):
    ...
    def on_new_session(data):
        ...
        if state["x2_mode"]:
            state["current_bet"] = state["base_bet_amount"] * (2 ** state["loss_streak"])
        else:
        ...
        if prediction:
            ...
            if state["auto_bet_enabled"]:
                if state["x2_mode"] and state["loss_streak"] > 0:
                    msg += f"⚡ <i>Đang gãy {state['loss_streak']} tay! X2 Nhồi tiền: <code>{state['current_bet']:,}</code> Win...</i>\n"
                else:
                    msg += f"⏳ <i>Vào cược: <code>{state['current_bet']:,}</code> Win...</i>\n"
        ...
        if state["current_prediction"]:
            if state["current_prediction"] == result:
                ...
                if state["waiting_for_result"]:
                    ...
                    state["loss_streak"] += 1
                    msg += f"❌ <b>GÃY CẦU!</b> Đang thua liên tiếp: {state['loss_streak']} tay (Sẽ x2 ở tay sau)\n"
...
# Giao diện nút bấm Menu Admin
def get_admin_markup():
    markup = telebot.types.InlineKeyboardMarkup(row_width=1)
    btn_add = telebot.types.InlineKeyboardButton("➕ Cấp Quyền Sử Dụng (Add VIP)", callback_data="admin_add_vip")
    btn_del = telebot.types.InlineKeyboardButton("➖ Xóa Quyền Sử Dụng (Del VIP)", callback_data="admin_del_vip")
    btn_list = telebot.types.InlineKeyboardButton("📋 Xem Danh Sách VIP", callback_data="admin_list_vip")
    btn_broadcast = telebot.types.InlineKeyboardButton("📢 Gửi Thông Báo Chung (Broadcast)", callback_data="admin_broadcast")
    markup.add(btn_add, btn_del, btn_list, btn_broadcast)
    return markup
...
def admin_menu(message):
    ...
    text = (
        "🛠 <b>BẢNG QUẢN TRỊ ADMIN (NÚT BẤM)</b>\n\n"
        "<i>Chọn chức năng bên dưới để quản lý hệ thống:</i>"
    )
    bot.reply_to(message, text, reply_markup=get_admin_markup(), parse_mode="HTML")
...
def send_welcome(message):
    ...
    if not is_vip(chat_id):
        return bot.reply_to(message, f"⛔ Bạn chưa được cấp quyền sử dụng bot.\nĐọc ID này gửi cho Admin để xin cấp quyền: <code>{chat_id}</code>", parse_mode="HTML")
...
def handle_callback_query(call):
    ...
    if data.startswith("admin_"):
        if chat_id not in ADMIN_IDS:
            return bot.answer_callback_query(call.id, text="⛔ Bạn không phải là Admin!", show_alert=True)
        
        init_user_state(chat_id)
        state = user_states[chat_id]

        if data == "admin_add_vip":
            state["awaiting_input"] = "admin_add"
            bot.answer_callback_query(call.id)
            bot.send_message(chat_id, "👉 Gửi ID số hoặc @username của người dùng muốn cấp quyền (Ví dụ: <code>123456789</code> hoặc <code>@nguoichoi</code>):", parse_mode="HTML")

        elif data == "admin_del_vip":
            state["awaiting_input"] = "admin_del"
            bot.answer_callback_query(call.id)
            bot.send_message(chat_id, "👉 Gửi ID số của người dùng muốn thu hồi/xóa quyền (Ví dụ: <code>123456789</code>):", parse_mode="HTML")

        elif data == "admin_list_vip":
            bot.answer_callback_query(call.id)
            text = "📋 <b>DANH SÁCH ID ĐƯỢC CẤP QUYỀN:</b>\n"
            for uid in vip_users:
                text += f"▪️ <code>{uid}</code>\n"
            bot.send_message(chat_id, text, parse_mode="HTML")

        elif data == "admin_broadcast":
            state["awaiting_input"] = "admin_broadcast_msg"
            bot.answer_callback_query(call.id)
            bot.send_message(chat_id, "👉 Gửi nội dung thông báo bạn muốn gửi đến <b>tất cả người dùng</b> bot:", parse_mode="HTML")
        return

    # --- XỬ LÝ CÁC NÚT USER THƯỜNG / VIP ---
    ...
    if data == "ui_login":
        ...
        if not state["x2_mode"]:
            state["loss_streak"] = 0
        status = "BẬT 🔥" if state["x2_mode"] else "TẮT 💤"
        bot.answer_callback_query(call.id, text=f"Đã {status} chế độ X2 Thua")
        bot.edit_message_reply_markup(chat_id, call.message.message_id, reply_markup=get_control_markup(chat_id))
        ...
        stat_msg = (f"📊 <b>THỐNG KÊ HIỆN TẠI</b>\n"
                    ...
                    f"🔥 X2 Thua: {'BẬT' if state['x2_mode'] else 'TẮT'} (Chuỗi thua: {state['loss_streak']})")
    ...
    if awaiting.startswith("admin_"):
        ...
        if awaiting == "admin_add":
            ...
            if text_input.startswith('@'):
                bot.reply_to(message, f"⚠️ Với username <code>{text_input}</code>, hãy chắc chắn họ đã bấm /start với bot ít nhất 1 lần. Khuyên dùng ID số để chính xác tuyệt đối.", parse_mode="HTML")
            else:
                try:
                    new_id = int(text_input)
                    vip_users.add(new_id)
                    bot.reply_to(message, f"✅ Đã cấp quyền thành công cho ID: <code>{new_id}</code>", parse_mode="HTML")
                except ValueError:
                    bot.reply_to(message, "❌ ID không hợp lệ! Vui lòng nhập dãy số.")
            return

        elif awaiting == "admin_del":
            state["awaiting_input"] = None
            try:
                del_id = int(text_input)
                if del_id in ADMIN_IDS:
                    return bot.reply_to(message, "❌ Không thể xóa quyền của Admin chính!")
                if del_id in vip_users:
                    vip_users.remove(del_id)
                    bot.reply_to(message, f"🗑️ Đã thu hồi quyền của ID: <code>{del_id}</code>", parse_mode="HTML")
                else:
                    bot.reply_to(message, "⚠️ ID này không có trong danh sách VIP.")
            except ValueError:
                bot.reply_to(message, "❌ ID không hợp lệ!")
            return

        elif awaiting == "admin_broadcast_msg":
            state["awaiting_input"] = None
            ...
            for uid in list(user_states.keys()):
                if isinstance(uid, int):
                    try:
                        bot.send_message(uid, msg_notice, parse_mode="HTML")
                        success_count += 1
                        time.sleep(0.1)
                    except Exception:
                        fail_count += 1
            
            bot.reply_to(message, f"✅ <b>Gửi broadcast hoàn tất!</b>\n"
                                  f"📨 Thành công: <code>{success_count}</code> người dùng\n"
                                  f"❌ Thất bại: <code>{fail_count}</code> người dùng", parse_mode="HTML")
            return

    # --- XỬ LÝ NHẬP LIỆU THƯỜNG / VIP ---
    if not is_vip(chat_id): return
