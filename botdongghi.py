...
def is_vip(chat_id):
    # ✅ KIỂM TRA CHAT_ID CÓ TRONG DANH SÁCH ADMIN HOẶC VIP
...
def handle_vip_commands(message):
    if message.chat.id not in ADMIN_IDS:  # ✅ KIỂM TRA NHIỀU ADMIN
        return
...
def send_welcome(message):
    if not is_vip(message.chat.id):
        return bot.reply_to(message, f"⛔ Đọc ID này cho Admin: <code>{message.chat.id}</code>", parse_mode="HTML")
